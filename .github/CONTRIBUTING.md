...
cross-compile matrix over the six release targets plus `plan9` and `wasip1`, which
ship nowhere and are built only to keep the `!unix` build tags honest,
`govulncheck`, and a `zizmor` audit of the
